> Правило ротации: в `docs/development-history.md` хранить только последние 10 записей; старые переносить в `docs/development-history-archive.md`.

## [YYYY-MM-DD HH:MM] - Краткое описание итерации

### Что сделано
- Конкретная задача 1
- Конкретная задача 2
- Конкретная задача 3

### Зачем
Краткое объяснение цели итерации и принятых решений

### Обновлено
- [ ] docs/architecture.md roadmap
- [ ] docs/adr.md (если есть)
- [ ] Тесты (если применимо)
- [ ] Документация

### Следующие шаги
- Что стоит сделать далее
